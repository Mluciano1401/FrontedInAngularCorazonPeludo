     ...
     async function populateTable() {
      try {
        ...
        data.data.forEach(item => {
          ...
          cellu.innerHTML = `
          ${(item.status) ? `<button id="deshabilitar-${item.tareaId}" class="btn btn-warning btn-sm me-1">Deshabilitar</button>` 
          : `<button id="habilitar-${item.tareaId}" class="btn btn-success btn-sm me-1">Habilitar</button>`}
          <button id="editar-${item.tareaId}" class="btn btn-primary btn-sm me-1"><a style="text-decoration: none;" href="../../../../public/enums/registrotarea.html?id${item.tareaId}">Editar</a></button>
          <button id="eliminar-${item.tareaId}" class="btn btn-danger btn-sm">Eliminar</button>
          `;
